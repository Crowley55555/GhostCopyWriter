...
ВАЖНО: Обычная Django аутентификация (регистрация/логин) отключена.
Единственный способ доступа - через временные токены.

Логика работы:
1. Пропускаем исключенные URL (admin, static, media, auth/token)
2. Проверяем наличие временного токена в сессии (access_token)
3. Если токен валиден - пропускаем с ограничениями (лимиты для DEMO)
4. Если токена нет или он невалиден - редирект на страницу требования токена
"""
...
class TokenAccessMiddleware:
    ...
    def __call__(self, request):
        ...
        # ПРИМЕЧАНИЕ: Обычная Django аутентификация отключена в пользу токенов
        # Единственный способ доступа - через временные токены
        
        # Проверяем наличие временного токена в сессии
